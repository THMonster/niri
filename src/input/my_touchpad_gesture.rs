...
impl State {
        ...
        if swipe.direction == GestureDirection::Horizontal {
            match swipe.cx {
                f64::NEG_INFINITY..-150.0 => {
                    swipe.cx = 0.;
                    match get_focused_app_id(&mut self.niri) {
                        App::Chrome => self.niri.my_touchpad_gesture.spawn(CHROME_LEFT_TAB),
                        App::Other => {
                ...
                _ => {
                    swipe.cx = 0.;
                    match get_focused_app_id(&mut self.niri) {
                        App::Chrome => self.niri.my_touchpad_gesture.spawn(CHROME_RIGHT_TAB),
                        App::Other => {
                    ...
                    if scale + 0.1 < pinch.scale {
                        ...
                        if let Some(mapped) = window {
                            ...
                            self.niri.layout.set_window_width(
                                Some(&w),
                                niri_ipc::SizeChange::AdjustProportion(-3.),
                            );
                        }
                    }
                }
                _ => {
                    if scale > pinch.scale + 0.15 {
                        pinch.scale = scale;
                        let window = self.niri.window_under_cursor();
                        if let Some(mapped) = window {
                            let w = mapped.window.clone();
                    ...
                    if scale + 0.1 < pinch.scale {
                        ...
                        match get_focused_app_id(&mut self.niri) {
                            ...
                            App::Other => {
                                ...
                                if let Some(mapped) = window {
                                    let w = mapped.window.clone();
                                    self.niri.layout.set_window_height(
                                        Some(&w),
                                        niri_ipc::SizeChange::AdjustProportion(-3.),
                                    );
                                }
                    ...
                    if scale > pinch.scale + 0.15 {
                        ...
                        match get_focused_app_id(&mut self.niri) {
                            ...
                            App::Other => {
                                ...
                                if let Some(mapped) = window {
                                    let w = mapped.window.clone();
                                    self.niri.layout.toggle_window_height(Some(&w), true);
                                }
                            }
                        }
                    }
    ...
    pub fn pinch_4f_on_end(&mut self, cancelled: bool) -> bool {
        ...
        match pinch.decision {
            ...
            GestureState::Decided => 'b1: {
                ...
                match pinch.scale {
                    0.0..0.7 => match get_focused_app_id(&mut self.niri) {
                        App::Chrome => self.niri.my_touchpad_gesture.spawn(CHROME_BACK),
                        App::Other => (),
                    },
                    0.7..1.3 => {}
                    _ => match get_focused_app_id(&mut self.niri) {
                        App::Chrome => self.niri.my_touchpad_gesture.spawn(CHROME_REFRESH),
                        App::Other => (),
                    },
                }
            }
    ...
    pub fn hold_4f_on_end(&mut self, ts: u32, cancelled: bool) -> bool {
        let hold = &mut self.niri.my_touchpad_gesture.hold_4f;
        if hold.decision != GestureState::Decided {
            return false;
        }
        if ts < hold.begin_ts + 300 {
        } else if cancelled {
        } else {
            match get_focused_app_id(&mut self.niri) {
                App::Chrome => self.niri.my_touchpad_gesture.spawn(CHROME_CLOSE_TAB),
                App::Other => {
                    let window = self.niri.window_under_cursor();
                    if let Some(mapped) = window {
...
fn get_focused_app_id(niri: &mut Niri) -> App {
    niri.layout.focus().map_or(App::Other, |x| {
        with_toplevel_role(x.toplevel(), |role| match role.app_id.as_deref() {
            Some("google-chrome") => App::Chrome,
            _ => App::Other,
        })
    })
}
